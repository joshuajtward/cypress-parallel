...
function parseScriptArguments(scriptArguments) {
  const parseFlags = (scriptArguments) => {
    const extraArgs = [];

    const isFlag = (value) => value?.[0] === '-';
    const isFlagAdded = (value) => isFlag(value) && extraArgs.includes(value);

    const args = scriptArguments.split(' ');
    args.forEach((value, i) => {
      isFlag(value) && !isFlagAdded(value)
        ? extraArgs.push(value)
        : extraArgs.push(parseFlagValue(value));
    });
  ...
  const parseFlagValue = (value) => {
    let varString = '';
    const keyValuePairs = value?.includes(',') ? value?.split(',') : [value];
    keyValuePairs.forEach((keyValuePair, i) => {
      const [key, val] = keyValuePair.split('=');
      varString += val.includes('"') ? `${key}=${val}` : `${key}="${val}"`;
      i + 1 !== keyValuePairs.length ? (varString += ',') : (varString += ' ');
    });
    return varString;
  };

  return parseFlags(scriptArguments);
}
...
function createCommandArguments(thread) {
  ...
  if (settings.scriptArguments)
    childOptions.push(...parseScriptArguments(settings.scriptArguments));
